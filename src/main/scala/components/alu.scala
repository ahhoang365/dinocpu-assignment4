...
class ALU extends Module {
  ...
  .elsewhen (io.operation === "b00001".U) { // sub
    io.result := io.operand1 - io.operand2
  }
  .elsewhen (io.operation === "b00010".U) { // mul
    io.result := io.operand1 * io.operand2
  }
  .elsewhen (io.operation === "b00011".U) { // div
  ...
  .elsewhen (io.operation === "b00100".U) { // rem
    when (io.operand2 === 0.U) { // division by zero
      io.result := io.operand1
    }
    .elsewhen ((io.operand1.asSInt === (((-1).S(64.W)) << 63)) && (io.operand2.asSInt === -1.S)) { // overflow
  ...
  .elsewhen (io.operation === "b00101".U) { // and
    io.result := io.operand1 & io.operand2
  }
  .elsewhen (io.operation === "b00111".U) { // or
    io.result := io.operand1 | io.operand2
  }
  .elsewhen (io.operation === "b01000".U) { // xor
    io.result := io.operand1 ^ io.operand2
  }
  .elsewhen (io.operation === "b01001".U) { // sra
    io.result := (io.operand1.asSInt >> io.operand2(5, 0)).asUInt // sra uses 6 bits of op2
  }
  .elsewhen (io.operation === "b01010".U) { // sll
    io.result := io.operand1 << io.operand2(5, 0) // sll uses 6 bits of op2
  }
  .elsewhen (io.operation === "b01011".U) { // srl
    io.result := io.operand1 >> io.operand2(5, 0) // srl uses 6 bits of op2
  }
  .elsewhen (io.operation === "b01100".U) { // slt
    io.result := (io.operand1.asSInt < io.operand2.asSInt).asUInt // signed operands
  }
  .elsewhen (io.operation === "b01101".U) { // divu
    when (io.operand2 === 0.U) { // division by zero
      io.result := (-1).S(64.W).asUInt
    }
  ...
  .elsewhen (io.operation === "b01110".U) { // remu
    when (io.operand2 === 0.U) { // division by zero
      io.result := io.operand1
    }
    .otherwise {
  ...
  .elsewhen (io.operation === "b10011".U) { // divw
  ...
  .elsewhen (io.operation === "b10100".U) { // remw
    when (operand2_32 === 0.U) { // division by zero
      io.result := signExtend32To64(operand1_32)
    }
    .elsewhen ((operand1_32.asSInt === (((-1).S(32.W)) << 31)) && (operand2_32.asSInt === -1.S)) { // overflow
  ...
  .elsewhen (io.operation === "b10101".U) { // mulh
    // The longer version
  ...
  .elsewhen (io.operation === "b10111".U) { // mulhu
  ...
  .elsewhen (io.operation === "b11110".U) { // remuw
    when (io.operand2 === 0.U) { // division by zero
      io.result := signExtend32To64(operand1_32)
    }
    .otherwise {
