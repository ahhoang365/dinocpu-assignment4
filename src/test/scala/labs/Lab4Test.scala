...
/**
  ...
  * }}}
  */
class SmallTestsPipelinedTesterLab4 extends CPUFlatSpec {
...
class FullApplicationsPipelinedTesterLab4 extends CPUFlatSpec {
  behavior of "Non-Combinational Pipelined CPU"
...
class SmallTestsSingleCycleTesterLab4 extends CPUFlatSpec {
  behavior of "Non-Combinational Single-Cycle CPU"
  for ((group, tests) <- InstTests.tests) {
    for (test <- tests) {
      it should s"run $group ${test.binary}${test.extraName}" in {
                CPUTesterDriver(test, "single-cycle-non-combin", "",
          "non-combinational", "non-combinational-port", 1) should be(true)
      }
    }
  }
...
// class FullApplicationsSingleCycleTesterLab4 extends CPUFlatSpec {
//   behavior of "Non-Combinational Single-Cycle CPU"
//   for (test <- InstTests.fullApplications) {
//     it should s"run full application ${test.binary}${test.extraName}" in {
//       CPUTesterDriver(test, "single-cycle-non-combin", "",
